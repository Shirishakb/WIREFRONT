...
const PageEditor = () => {
  ...
  const [page, setPage] = useState<{ name: string; _id: string } | null>(null);
  ...
  const [editingComponent, setEditingComponent] = useState<Component | null>(null); // The component being edited
  ...
  const handleSavePageName = async () => {
    if (page) {
      const updatedPage = await updatePage(page._id, newPageName, page.width, page.height);
      if (updatedPage) {
        setPage(updatedPage);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', padding: '20px', fontFamily: 'Arial, sans-serif' }}>
      ...
      <h2 style={{ color: '#4CAF50' }}>Interactive Page Editor</h2>
       ...
       {isEditingPageName ? (
        ...
        <div>
          <h3>{page?.name}</h3>
          <button onClick={() => setIsEditingPageName(true)}>Edit Name</button>
        </div>
      )}
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)}>
        ...
        <Modal.Footer>
          ...
          <Button
            ...
            onClick={() => {
              if (editingComponent) {
                saveComponentProperties(editingComponent, optionValue);
              }
              setShowModal(false);
            }}
          >
